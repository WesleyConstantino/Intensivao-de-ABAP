...
FORM zf_exibe_alv_poo.
  ...
  TRY.
      ...
      lo_table->get_columns( )->get_column( 'MATNR' )->set_short_text( 'Wesley' ). "Mudar o texto curto da tabela
      lo_table->get_columns( )->get_column( 'MATNR' )->set_medium_text( 'Wesley' ).
      lo_table->get_columns( )->get_column( 'MATNR' )->set_long_text( 'Wesley' ).
      ...
      lo_header->create_text( row     = 3
                              column  = 1
                              text    = 'Horas: ' && sy-uzeit ). "Concatenação com "&&"

      lo_header->create_text( row     = 3  " Linha
                              column  = 4  " Coluna
                              text    = 'Data: ' && sy-datum ). "Mostra data
